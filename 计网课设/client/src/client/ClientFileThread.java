...
public class ClientFileThread extends Thread{
    ...
    public void run() {
        try {
            ...
            socket = new Socket(hostIp, 8090);  // 客户端套接字
...
//					out.flush();
                    File topFile = new File(com+"\\接受文件");
                    if(!topFile.exists()) {  // 新建接受文件夹
                        topFile.mkdir();
                    }
                    File userFile = new File(com+"\\接受文件\\" + userName);
                    if(!userFile.exists()) {  // 新建当前用户的文件夹
                        userFile.mkdir();
                    }
                    File file = new File(com+"\\接受文件\\" + userName + "\\"+ textName);
...
//						out.flush();
                        if(curLength == totleLength) {  // 强制结束
                            out.println("TOALL@" + frame.getTitle() + "@" + selectUser + "@" +"【" + userName + "接收了文件！】");
                            out.flush();
                            break;
                        }
                    }
    ...
    static void outFileToServer(String path) {
        try {
            ...
            String selectUser = "所有人";
            out.println("TOALL@" + frame.getTitle() + "@" + selectUser + "@" +"【" + userName + "已成功发送文件！】");
            out.flush();
        } catch (Exception e) {}
    }
}
